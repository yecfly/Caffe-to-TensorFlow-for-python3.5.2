...
class Graph(object):
    ...
    def __str__(self):#showing the networks, it's dispensable
        ...
        for node in self.topologically_sorted():
            ...
            #data_shape = node.data[0].shape if node.data else '--'#In python3.x TypeError: 'map' object is not subscriptable
            if node.data:
            ...
            if node.output_shape:
               out_shape='(%d, %d, %d, %d)'%(node.output_shape[0],node.output_shape[1],node.output_shape[2],node.output_shape[3])
               #print(out_shape)
            else:
               out_shape='--'
            #s.append('{:<20} {:<30} {:>20} {:>20}'.format(node.kind, node.name, data_shape,
            #                                              tuple(out_shape)))
            s.append('{:<20} {:<30} {:>20} {!s:>20}'.format(node.kind, node.name, data_shape, out_shape))
            
        return '\n'.join(s)

def retainType(layer):
    '''Retain Type. Increase the compatibilities of caffe network prototxt. ...'''
    if(isinstance(layer.type, str)):
        return layer.type
    else:
        listofpara=str(layer).splitlines()
        for li in listofpara:
            if li.find('type:')>-1:
                return li.replace("type: ","")

        return None
        #print("\nTYPE: %s"%reextracttype)
...
class GraphBuilder(object):
    ...
    def load(self):
        ...
        #with open(self.def_path, 'rb') as def_file: #works OK in python 2.x, but not in python 3.x
    ...
    def make_node(self, layer):
        ...
        print("\nProcessing layer: %s ..."%layer.name)
        layerNodeType=retainType(layer)
        print("Re-extract TYPE: %s"%layerNodeType)
        kind = NodeKind.map_raw_kind(layerNodeType)
        print("Node Kind: %s"%kind)
        if kind is None:
            print("\nTrying to find: \n%s\n%s\n%s\n%s\n%s\n%s"%(layer,layer.bottom,layer.top,layer.name,layer.type,layer.convolution_param))
