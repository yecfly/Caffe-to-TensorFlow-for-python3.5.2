...
class DataInjector(object):
    ...
    def adjust_parameters(self, node, data):
        ...
        if node.kind == NodeKind.INNER_PRODUCT or node.kind == NodeKind.InnerProduct:
...
class DataReshaper(object):
    ...
    def __call__(self, graph):
        for node in graph.nodes:
            ...
            if ((node.kind == NodeKind.INNER_PRODUCT) or (node.kind == NodeKind.InnerProduct) ) and self.has_spatial_parent(node):
...
class ReLUFuser(SubNodeFuser):
    ...
    def is_eligible_pair(self, parent, child):
        ...
        return ((self.allowed_parent_types is None or parent.kind in self.allowed_parent_types) and
                ((child.kind == NodeKind.RELU) or (child.kind == NodeKind.ReLU)))
...
class BatchNormScaleBiasFuser(SubNodeFuser):
    ...
    def is_eligible_pair(self, parent, child):
        ...
        return (((parent.kind == NodeKind.BATCH_NORM) or (parent.kind == NodeKind.BatchNorm)) and 
                ((child.kind == NodeKind.SCALE) or (child.kind == NodeKind.Scale)) and
                child.parameters.axis == 1 and child.parameters.bias_term == True)
...
class BatchNormPreprocessor(object):
    ...
    def __call__(self, graph):
        for node in graph.nodes:
            if (node.kind != NodeKind.BATCH_NORM) or (node.kind != NodeKind.BatchNorm) :
                continue
...
class ParameterNamer(object):
    ...
    def __call__(self, graph):
        for node in graph.nodes:
            if node.data is None:
                continue
            if node.kind in (NodeKind.CONVOLUTION, NodeKind.INNER_PRODUCT, NodeKind.Convolution, NodeKind.InnerProduct):
                names = ('weights',)
                if node.parameters.bias_term:
                    names += ('biases',)
            elif node.kind == NodeKind.BATCH_NORM or node.kind == NodeKind.BatchNorm:
                names = ('mean', 'variance')
                if len(node.data) == 4:
                    names += ('scale', 'offset')
            else:
